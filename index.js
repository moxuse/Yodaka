...
(function($PS) {
  // Generated by purs version 0.13.3
  "use strict";
  $PS["Graphics.Yodaka.Node.RGBNoisePlane"] = $PS["Graphics.Yodaka.Node.RGBNoisePlane"] || {};
  var exports = $PS["Graphics.Yodaka.Node.RGBNoisePlane"];
  var Graphics_Three_GeometryAddition = $PS["Graphics.Three.GeometryAddition"];
  var Graphics_Three_Material = $PS["Graphics.Three.Material"];
  var Graphics_Three_Math_Vector = $PS["Graphics.Three.Math.Vector"];
  var Graphics_Three_Object3D = $PS["Graphics.Three.Object3D"];                
  ...
  var fragmentalShader = "\x0a  uniform vec3 resolution;\x0a\x0a  vec2 random(vec2 st) {\x0a    st = vec2( dot(st, vec2(127.1, 311.7)),\x0a              dot(st, vec2(269.5, 183.3)) );\x0a              \x0a    return 2.0 * fract(sin(st) * 43758.5453123) - 1.0;\x0a  }\x0a\x0a\x0a  float noise(vec2 st) {\x0a    vec2 i = floor(st);\x0a    vec2 f = fract(st);\x0a\x0a    vec2 u = f * f * (3.0 - 2.0 * f);\x0a\x0a    return mix( mix( dot( random(i + vec2(0.0,0.0) ), f - vec2(0.0,0.0) ), \x0a                     dot( random(i + vec2(1.0,0.0) ), f - vec2(1.0,0.0) ), u.x),\x0a                mix( dot( random(i + vec2(0.0,1.0) ), f - vec2(0.0,1.0) ), \x0a                     dot( random(i + vec2(1.0,1.0) ), f - vec2(1.0,1.0) ), u.x), u.y);\x0a  }\x0a\x0a  void main() {\x0a    vec2 st = gl_FragCoord.xy / resolution.xy;\x0a    vec3 color = vec3(0.0);\x0a    vec2 posX= vec2(st * 10.0);\x0a    vec2 posY = vec2(st * 20.0);\x0a    vec2 posZ = vec2(st * 30.0);\x0a    color = vec3( noise(posX) * .85 + .15, noise(posY) * .85 + .15, noise(posZ) * .85 + .15 );\x0a    gl_FragColor = vec4(color, 1.0);\x0a  }\x0a";
...
(function($PS) {
  ...
  var Graphics_Yodaka_Node_RGBNoisePlane = $PS["Graphics.Yodaka.Node.RGBNoisePlane"];
  ...
  var main = function __do() {
      ...
      var v = Graphics_Yodaka_Context.render(Graphics_Yodaka_Node_RGBNoisePlane.rgbNoisePlane)();
      var v1 = Graphics_Yodaka_Context.render(Graphics_Yodaka_Node_RGBNoisePlane.rgbNoisePlane)();
      var v2 = Graphics_Yodaka_Context.render(Graphics_Yodaka_Node_NoisePlane.noisePlane)();
